Shader "GS202/Lesson3/UseParallax"
{
    Properties {
        ...
        // 高低是平的，默认值是(.5,.5,.5)，所以视差贴图默认是灰色
        _ParallaxMap("Parallax Map", 2D) = "gray" {}
        _ParallaxRatio("Parallax Ratio", Range(.001, .08)) = .01
    }
    SubShader
    {
        Pass {
            ...
            fixed4 frag(v2f i) : SV_TARGET {
                float3x3 rotation = float3x3(i.worldT, i.worldB, i.worldNorm);
                half3 viewDir = normalize(UnityWorldSpaceViewDir(i.worldPos));
                half3 nSpaceView = normalize(mul(rotation, viewDir));
                // Parallax
                half h = tex2D(_ParallaxMap, i.uv).g;
                float2 offset = ParallaxOffset(h, _ParallaxRatio, nSpaceView);
                i.uv += offset;
                // Bump
                ...
                half3 worldNormal = normalize(mul(unpackedNormal, rotation));
                // 采样
